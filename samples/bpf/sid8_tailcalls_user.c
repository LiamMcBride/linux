...
int main(int argc, char **argv)
{
    ...
    tailcall_prog = bpf_object__find_program_by_name(obj, "testing_func");
    if(!prog){
...
//    int test_map = bpf_object__find_map_fd_by_name(obj, "test");
//    if(test_map<0){
//        fprintf(stderr, "ERROR: finding the test map in the object file failed\n");
//        goto cleanup;
//    
//    }

//    void* value; 
//    bpf_map_lookup_elem(test_map, &key, &value);    
    char pfilename[256];
    int len = snprintf(pfilename, 256, "%s/%s", BPF_SYSFS_ROOT, "tailcall_prog"); 
    bpf_program__pin(tailcall_prog, pfilename);
    
    int my_map = bpf_object__find_map_fd_by_name(obj, "prog_array_init");
    if(my_map<0){
    ...
    //int tail_prog_fd = bpf_program__fd(tailcall_prog);
//    int my_map_fd = bpf_program__fd(my_map);
    //int key = 1;
    //bpf_map_update_elem(my_map, &key, &tail_prog_fd, 0);
