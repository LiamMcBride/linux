...
int testing_func(void *ctx){
    ...
    //bpf_printk("callee beginning stack pointer %lx\n", x);

    unsigned char a[STACKSIZE] = {0};

    for(int i = 0; i < STACKSIZE; i++) {
        a[i] = i % 255;
    }   
    bpf_printk("value of a[0]: %d", a[0]);  
    //unsigned long y = bpf_get_stack_size();

    bpf_printk("callee beginning stack pointer %px\n", &a[STACKSIZE-1]);
    bpf_printk("callee ending stack pointer %px\n",&a[0]);
...
SEC("fentry/__x64_sys_execve")
int testing_func_kern(void *ctx){
    bpf_printk("No map update from userspace");
    return 0;
}
...
static __attribute__((__noinline__)) int testing_bpf_to_bpf_calls(void *ctx){
    ...
    bpf_printk("bpf-to-bpf beginning stack pointer %px\n", &a[STACKSIZE]);
    bpf_printk("bpf-to-bpf ending stack pointer %px\n",&a[0]);
...
{	
   ...
   // unsigned long x = bpf_get_stack_size();
    ...
    bpf_printk("caller beginning stack pointer %px\n", &a[STACKSIZE]);
    bpf_printk("caller ending stack pointer %px\n",&a[0]);
